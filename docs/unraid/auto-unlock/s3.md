...
Other S3-compatible providers (e.g., MinIO, Wasabi, Backblaze) follow similar steps. Example configuration strings:
...
Using the Backblaze B2 provider:
...
```text
:s3,provider=Other,access_key_id=KEY_ID,secret_access_key=APPLICATION_KEY,endpoint=YOUR_ENDPOINT:BUCKET_NAME/piece.txt
```

### Add to Automatic Disk Unlock Configuration
